	...
	if(symbol->option_2 == 0) { /* The size of the symbol can be determined by Zint */
		do {
			...
			switch(ecc_level) {
				...
				case 1: for(_i = 32; _i > 0; _i--) {
						if((data_length + adjustment_size) < Aztec10DataSizes[_i - 1]) {
							layers = _i;
							compact = 0;
							data_maxsize = Aztec10DataSizes[_i - 1];
						}
					}
					for(_i = comp_loop; _i > 0; _i--) {
						if((data_length + adjustment_size) < AztecCompact10DataSizes[_i - 1]) {
							layers = _i;
							compact = 1;
							data_maxsize = AztecCompact10DataSizes[_i - 1];
						}
					}
					break;
				case 2: for(_i = 32; _i > 0; _i--) {
						if((data_length + adjustment_size) < Aztec23DataSizes[_i - 1]) {
							layers = _i;
							compact = 0;
							data_maxsize = Aztec23DataSizes[_i - 1];
						}
					}
					for(_i = comp_loop; _i > 0; _i--) {
						if((data_length + adjustment_size) < AztecCompact23DataSizes[_i - 1]) {
							layers = _i;
							compact = 1;
							data_maxsize = AztecCompact23DataSizes[_i - 1];
						}
					}
					break;
				case 3: for(_i = 32; _i > 0; _i--) {
						if((data_length + adjustment_size) < Aztec36DataSizes[_i - 1]) {
							layers = _i;
							compact = 0;
							data_maxsize = Aztec36DataSizes[_i - 1];
						}
					}
					for(_i = comp_loop; _i > 0; _i--) {
						if((data_length + adjustment_size) < AztecCompact36DataSizes[_i - 1]) {
							layers = _i;
							compact = 1;
							data_maxsize = AztecCompact36DataSizes[_i - 1];
						}
					}
					break;
				case 4: for(_i = 32; _i > 0; _i--) {
						if((data_length + adjustment_size) < Aztec50DataSizes[_i - 1]) {
							layers = _i;
							compact = 0;
							data_maxsize = Aztec50DataSizes[_i - 1];
						}
					}
					for(_i = comp_loop; _i > 0; _i--) {
						if((data_length + adjustment_size) < AztecCompact50DataSizes[_i - 1]) {
							layers = _i;
							compact = 1;
							data_maxsize = AztecCompact50DataSizes[_i - 1];
						}
					}
					break;
			}
